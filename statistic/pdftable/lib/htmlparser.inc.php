...
$HTML_ATTRIBUTE_STAND_ALONE = ['checked', 'compact', 'declare', 'defer', 'disabled', 'ismap', 'multiple', 'nohref', 'noresize', 'noshade', 'nowrap', 'readonly', 'selected'];
...
{
    public function HTMLFileParser($fileName)
    {
        $fp = fopen($fileName, 'r');
        $content = '';
        while (true) {
...
{
    public function HTMLURLParser($url)
    {
        $fp = fopen($url, 'r');
        $content = '';
        while (true) {
...
class TreeHTML
{
     ...
     */
    public function TreeHTML($parser, $file = true)
    {
        $i = 0;
        if ($file) {
            while ($parser->parse()) {
     ...
     */
    public function replace($name, $text)
    {
        $index = array_search($name, $this->field);
        if (!$index) {
            return;
     ...
     */
    public function setReadonlyAll()
    {
