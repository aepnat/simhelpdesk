...
$PDF_ALIGN = ['left'=>'L', 'center'=>'C', 'right'=>'R', 'justify'=>'J'];
...
public $defaultFontFamily;
    ...
    public $isNotYetSetFont;
    public $headerTable;
    public $footerTable;
    public $paddingCell = 1; //(mm)
public $paddingCell2 = 2; //2*$paddingCell
public $spacingLine = 0; //(mm)
public $spacingParagraph = 0; //(mm)

public function PDFTable($orientation = 'P', $unit = 'mm', $format = 'A4')
{
    ...
    public function SetPadding($s = 1)
    {
        $this->paddingCell = $s;
        $this->paddingCell2 = 2 * $s;
    }

    public function SetSpacing($linespacing = 1, $paragraphspacing = 2)
    {
     ...
     */
    public function getLineHeight($fontSize = 0)
    {
        if ($fontSize == 0) {
    ...
    private function _tableParser(&$html)
    {
        ...
        foreach ($t->name as $i=>$element) {
            ...
            && (in_array($element, ['table', 'tr', 'td', 'font']))) {
                $fontopen = false;
            }
            if ($tdopen && $t->type[$i] == NODE_TYPE_ENDELEMENT
            && (in_array($element, ['table', 'tr', 'td']))
        ...
        $table['cells'] = $cell;
        $table['wc'] = array_pad([], $table['nc'], ['miw'=>0, 'maw'=>0]);
        $table['hr'] = array_pad([], $table['nr'], 0);
    ...
    private function _tableWriteRow(&$table, $i, $x0)
    {
        ...
        for ($j = 0; $j < $table['nc']; $j++) {
            if (isset($table['cells'][$i][$j]['miw'])) {
                ...
                //Border
                if (isset($c['border'])) {
                    $table['listborder'][] = [$x, $y, $w, $h, $c['border']];
                } elseif (isset($table['border']) && $table['border']) {
                    $table['listborder'][] = [$x, $y, $w, $h, $table['border']];
                }
            }
        }
