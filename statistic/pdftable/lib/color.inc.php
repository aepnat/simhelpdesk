...
class Color
{
     ...
     */
    public function _HSL_2_RGB($v1, $v2, $vH)
    {
        if ($vH < 0) {
            $vH += 1;
        }
     ...
     */
    public function HSL2RGB($H, $S, $L, &$R, &$G, &$B)
    {
        if ($S == 0) {
            $R = $L * 255;
            $G = $L * 255;
            ...
            $G = 255 * self::_HSL_2_RGB($var_1, $var_2, $H);
            $B = 255 * self::_HSL_2_RGB($var_1, $var_2, $H - (1 / 3));
        }
    }
     ...
     */
    public function getListColor($n)
    {
        $h = 0;
        $s = 1;
        $l = 0.6;
