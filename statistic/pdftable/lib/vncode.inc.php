...
class VNCode
{
     ...
     */
    public function _nextCode($str, &$index)
    {
        if ($index >= strlen($str)) {
            return 0;
        }
     ...
     */
    public function UTF8_NCR($str)
    {
        $result = '';
        $len = strlen($str);
        for ($i = 0; $i < $len;) {
            $code = self::_nextCode($str, $i);
     ...
     */
    public function indexUTF8($data, $index)
    {
        $len = strlen($data);
        if ($len == 0) {
            return 0;
     ...
     */
    public function correctUTF8($data)
    {
        $len = strlen($data);
        if ($len >= 3) {
            $code = ord($data[$len - 3]);
     ...
     */
    public function correctNCR($data)
    {
        for ($i = 128; $i < 256; $i++) {
     ...
     */
    public function trunstrword($str, $trunsize = 0)
    {
        ...
        $len = strlen($str);
        for ($r = self::indexUTF8($str, $size); $r < $len && $str[$r] != ' ' && $r < $size + 10; $r++);
     ...
     */
    public function indexNCR($data, $index)
    {
        $p = 0;
        $len = strlen($data);
     ...
     */
    public function strlenNCR($data)
    {
        $p = 0;
        $len = strlen($data);
        for ($i = 0; $i < $len; $i++) {
